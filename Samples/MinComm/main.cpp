...
//AAEON add for RS485/RS232 mode switch
UCHAR hexEnabled = FALSE;
UCHAR cmdLoRa = 0;
#define IOCTL_GET_PORT_MODE \
    CTL_CODE(FILE_DEVICE_SERIAL_PORT, 0x0B0E, METHOD_BUFFERED, FILE_ANY_ACCESS)

#define IOCTL_SET_PORT_MODE \
    CTL_CODE(FILE_DEVICE_SERIAL_PORT, 0x0B0F, METHOD_BUFFERED, FILE_ANY_ACCESS)	

typedef struct _PORT_MODE {
	UCHAR Mode;
	UCHAR Is485;
	UCHAR Invert;
} PORT_MODE, *PPORT_MODE;
...
struct SerialParamMask {
    ...
    ULONG IdsrSet : 1;
	//AAEON add 
	UCHAR Mode;	//1:RS232 //2:RS485(TX enable with RTS low) //3:RS485(TX enable with RTS high)
	UCHAR Rs485;
};
...
void SendLoRaCommand(HANDLE serialHandle)
{
	CHAR rDevEUI[3] = { 0xf4,0xd0,0xd3 };
	CHAR rAppEUI[3] = { 0x53,0xd0,0xf0 };
	CHAR rAppKEY[3] = { 0x64,0xd0,0xb0 };
	CHAR buf[512];
...
void ReadSerialWriteConsole (HANDLE serialHandle)
{
    ...
    for (;;) {
		...
		if (hexEnabled)
		{
			...
			{
				sprintf_s(hbuf+i*3,sizeof(hbuf),"%.2X ", (UCHAR)buf[i]);	//convert to UCHAR to avoid FFFFFF
			}
			//continue;
		}
		//printf("bytesRead=%d\n", bytesRead);
        ...
        int charsConverted = MultiByteToWideChar(
                ...
                MB_PRECOMPOSED,
				hexEnabled?hbuf:buf,
				hexEnabled?bytesRead*3:bytesRead,
                wbuf,
                ARRAYSIZE(wbuf));
        if (!charsConverted) {
            throw wexception(
    ...
    )
{
    *MaskPtr = SerialParamMask();
    ...
    for (int i = 0; i < argc; ++i) {
		...
		if (value == L"1")	//read DEVEUI
			cmdLoRa = 1;
		else if (value == L"2") //read APPEUI
			cmdLoRa = 2;
		else if (value == L"3") //read APPKEY
			cmdLoRa = 3;
		else
			cmdLoRa = 0;
		}
...
void RunSerialConsole (PCWSTR DevicePath, _In_ DCB* DcbPtr, SerialParamMask ParamMask)
{
	...
	//AAEON add for RS485/RS232 mode switch
	int Result = 99;
	unsigned long returnSize = 10;
	PORT_MODE mode1;
	mode1.Mode = ParamMask.Mode;
	mode1.Is485 = ParamMask.Rs485;
	Result = DeviceIoControl(serialHandle.Get(), IOCTL_SET_PORT_MODE,
		&mode1, sizeof(PORT_MODE),
		&mode1, sizeof(PORT_MODE),
		&returnSize, NULL);
	...
	//Send LoRa Command
	SendLoRaCommand(serialHandle.Get());
...
L"                       or off.\n"
L"  hex={on|off}		 on/off hex data sned and display .\n"
L"  lora={1|2|3}		 1:read DevEUI 2:read AppEUI 3:read AppKEY .\n"
L"  mode={1|2|3}         1:RS232 2:RS485(TX enable with RTS low) 3:RS485(TX enable with RTS high)\n"
L"  rs485={on|off}       on/off RS485 mode(Fintek only)\n"
L"\n"
