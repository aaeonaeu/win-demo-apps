...
SpiDevice^ lgw_spi_target; /*! generic pointer to the SPI device */
...
int reg_w_align32(SpiDevice^ spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, struct lgw_reg_s r, int32_t reg_value) {
...
int reg_r_align32(SpiDevice^ spi_target, uint8_t spi_mux_mode, uint8_t spi_mux_target, struct lgw_reg_s r, int32_t *reg_value) {
...
int lgw_connect(bool spi_only, uint32_t tx_notch_freq) {
    ...
    if (spi_stat != LGW_SPI_SUCCESS) {
		...
		DEBUG_MSG("INFO: SPI PORT OPENED\n");
...
int lgw_soft_reset(void) {
    ...
    if (!lgw_spi_target || (lgw_regpage < 0)) {
