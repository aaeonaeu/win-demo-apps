        ...
        static async Task LedSetting(string led, string cmd)
        {
            ...
            if (lamp != null)
            {
                ...
                else
                {
                    ...
                    //compare Led color
                    foreach (var color in typeof(System.Drawing.Color).GetProperties())
                    {
                        if (led.ToLower().Contains(color.Name.ToLower()))
                        {
                            System.Drawing.Color state = System.Drawing.Color.FromName(color.Name);
                            lamp.Color = Windows.UI.Color.FromArgb(state.A, state.R, state.G, state.B);
