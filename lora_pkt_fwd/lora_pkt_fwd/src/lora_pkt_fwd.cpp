...
bool exit_sig = false; /* 1 -> application terminates cleanly (shut down hardware, close open files, etc) */
bool quit_sig = false; /* 1 -> application terminates without shutting down the hardware */
...
static int send_tx_ack(uint8_t token_h, uint8_t token_l, enum jit_error_e error) {
    ...
    *(uint32_t *)(buff_ack + 8) = net_mac_l;
    ...
    return send(sock_down, (const char*)buff_ack, buff_index, 0);//send(sock_down, (void *)buff_ack, buff_index, 0);
}
...
int main(Platform::Array<Platform::String^>^ args)
{
    ...
    const char *global_cfg_path= "global_conf.json"; /* contain global (typ. network-wide) configuration */
    const char *local_cfg_path = "local_conf.json"; /* contain node specific configuration, overwrite global parameters for parameters that are defined in both */
    const char *debug_cfg_path = "debug_conf.json"; /* if present, all other configuration files are ignored */
    ...
    if (_access(debug_cfg_path, R_OK) == 0) { /* if there is a debug conf, parse only the debug conf */
    ...
    } else if (_access(global_cfg_path, R_OK) == 0) { /* if there is a global conf, parse it and then try to parse local conf  */
        ...
        if (_access(local_cfg_path, R_OK) == 0) {
    ...
    } else if (_access(local_cfg_path, R_OK) == 0) { /* if there is only a local conf, parse it and that's all */
        MSG("INFO: found local configuration file %s, parsing it\n", local_cfg_path);
    ...
    if (gps_tty_path[0] != '\0') { /* do not try to open GPS device if no path set */
        i = lgw_gps_enable(gps_tty_path, (char*)"ubx7", 0, &gps_tty_fd); /* HAL only supports u-blox 7 for now */
...
void thread_up(void) {
    ...
    struct tm * x=NULL; /* broken-up UTC time */
    ...
    i = setsockopt(sock_up, SOL_SOCKET, SO_RCVTIMEO, (const char *)&push_timeout_half, sizeof push_timeout_half);
    if (i != 0) {
    ...
    *(uint32_t *)(buff_up + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        for (i=0; i < nb_pkt; ++i) {
            ...
            if (ref_ok == true) {
                ...
                if (j == LGW_GPS_SUCCESS) {
					...
					gmtime_s(x,&(pkt_utc_time.tv_sec));
        ...
        /* send datagram to server */
        send(sock_up, (const char *)buff_up, buff_index, 0);
        ...
        for (i=0; i<2; ++i) {
            j = recv(sock_up, (char *)buff_ack, sizeof buff_ack, MSG_INTERRUPT);
...
void thread_down(void) {
    ...
    i = setsockopt(sock_down, SOL_SOCKET, SO_RCVTIMEO, (const char *)&pull_timeout, sizeof pull_timeout);
    if (i != 0) {
    ...
    *(uint32_t *)(buff_req + 8) = net_mac_l;
    ...
    while (!exit_sig && !quit_sig) {
        ...
        send(sock_down, (const char *)buff_req, sizeof buff_req, 0);
        clock_gettime(CLOCK_MONOTONIC, &send_time);
        pthread_mutex_lock(&mx_meas_dw);
        meas_dw_pull_sent += 1;
        pthread_mutex_unlock(&mx_meas_dw);
        ...
        while ((int)difftimespec(recv_time, send_time) < keepalive_time) {
			...
			msg_len = recv(sock_down, (char *)buff_down, (sizeof buff_down) - 1, MSG_INTERRUPT);
            clock_gettime(CLOCK_MONOTONIC, &recv_time);
...
void thread_gps(void) {
    ...
    while (!exit_sig && !quit_sig) {
        ...
        size_t nb_char = read(gps_tty_fd, serial_buff + wr_idx, LGW_GPS_MIN_MSG_SIZE);
        if (nb_char <= 0) {
